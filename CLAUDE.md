...
| `shared/switches.js` | Learning/reading switches: keys + Gamepad API. Numbered switch list for Scenes (min 5 in Scenes, max 12, id + colour); `capture()` for one-off learning (Activities' scanning switches). `ROOM`: the sensory room's SimplyWorks switches (gamepad buttons 12–16 for the red/yellow/green/blue/white box, 18 for the single white), used when nothing is learned and by `useRoom()`; ready-made presets have six jobs to match |
